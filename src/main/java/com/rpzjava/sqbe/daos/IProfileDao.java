...
public interface IProfileDao extends JpaRepository<UserProfile,Long> {
    ...
    @Transactional
    @Modifying//分好后不要有空格
    @Query(value="update UserProfile set bio =:bio,avatar =:avatar, nickName =:name, sex =:sex where id =(select uid from UserEntity where sicnuid =:sicnuid)")
    int updateData(@Param("sicnuid") String sicnuid,@Param("avatar") byte[] avatar,@Param("bio") String bio,@Param("nick_name") String name,@Param("sex") String sex);
